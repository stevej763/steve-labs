...
type Post = { id: string; slug: string; title: string; excerpt: string | null; body: string; status: "DRAFT" | "PUBLISHED"; publishedAt: string | null; featuredMediaId: string | null; featuredImageUrl: string | null; tags: string[] };
...
const emptyDraft: Draft = { id: "new", slug: "", title: "", excerpt: "", body: "", featuredMediaId: null, featuredImageUrl: null, tags: [] };
...
export default function Home() {
  ...
  function toDraft(post: Post): Draft {
    return { id: post.id, slug: post.slug, title: post.title, excerpt: post.excerpt, body: post.body, featuredMediaId: post.featuredMediaId, featuredImageUrl: post.featuredImageUrl, tags: post.tags };
  }
  ...
  return (
    <main className="admin-shell">
      ...
      <section className="content-panel" aria-labelledby="posts-heading">
        ...
        {draft && <label className="tag-editor">Tags<input value={draft.tags.join(", ")} onChange={(event) => setDraft({ ...draft, tags: [...new Set(event.target.value.split(",").map((tag) => tag.trim().toLowerCase()).filter(Boolean))] })} placeholder="e.g. java, spring-boot" /></label>}
